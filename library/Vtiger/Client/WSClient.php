...
class Vtiger_Client_WSClient {
	...
	function __construct($url) { 
		...
		$this->_client = new Vtiger_Client_HTTP_Client($this->_serviceurl);
	}
	 ...
	 * Do Query Operation.
	 */
	function doQuery($query, $debug = false) {
		...
		$resultdata = $this->_client->doGet($getdata, true, 20, $debug);
		if($this->hasError($resultdata)) {
			return false;
		}
	 ...
	 * Do Create Operation
	 */
	function doCreate($module, $valuemap, $debug = false) {
		// Perform re-login if required.
		$this->__checkLogin();
		...
		$resultdata = $this->_client->doPost($postdata, true, 20, $debug);
	 ...
	 * Do Create Operation
	 */
	function doUpdate($object, $debug = false) {
		// Perform re-login if required.
		$this->__checkLogin();
		...
		$postdata = Array(
			'operation'   => 'update',
			'sessionName' => $this->_sessionid,
			'element'     => $this->toJSONString($object)
		);
	 ...
	 */
	function doInvoke($method, $params = null, $type = 'POST', $debug = false) {
		// Perform re-login if required
		$this->__checkLogin();
		...
		if(strtoupper($type) == "POST") {
			$resultdata = $this->_client->doPost($senddata, true, 20, $debug);
		} else {
			$resultdata = $this->_client->doGet($senddata, true, 20, $debug);
		}
